...
function switchTurn() {
  ...
  if (status == gameStatus.MORE_MOVES_LEFT) {
    ...
    playerTurn = false;
    if (status == 2) document.getElementById("turnInfo").innerHTML = "You win!";
    else if (status == 3)
      document.getElementById("turnInfo").innerHTML = "Computer wins!";
    else document.getElementById("turnInfo").innerHTML = "Draw game";
  }
}
...
function makeComputerMove() {
  ...
  while (buttons[move].hasAttribute("disabled")) {
    move = Math.floor(Math.random() * 9);
  }
  buttons[move].innerHTML = "O";
  buttons[move].classList.add("o");
  buttons[move].setAttribute("disabled", "");
  switchTurn();
}
